...
async def lifespan(app: FastAPI):
    ...
    app.state.engine = GameEngine(checkpointer)
...
async def join_room(code: str, body: NameBody):
    ...
    async with app.state.sessions() as session:
        count = await rooms.player_count(session, code)
        if not rooms.can_join(count):
        ...
        info, token = rooms.new_player(name, seat=count)
